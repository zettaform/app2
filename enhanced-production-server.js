...
function validateEnvironment() {
  const required = ['AWS_ACCESS_KEY_ID', 'AWS_SECRET_ACCESS_KEY', 'JWT_SECRET'];
  ...
  console.log(`🔧 AWS Region: ${process.env.AWS_REGION || process.env.AWS_DEFAULT_REGION || 'us-east-1'}`);
  console.log(`🗄️  Tables configured:`, Object.values(TABLES));
  return true;
}
...
const dynamoClient = new DynamoDBClient({
  ...
  } : undefined, // Let AWS SDK use default credential chain if not provided
  maxAttempts: 3,
  retryMode: 'adaptive'
});
