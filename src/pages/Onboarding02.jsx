...
function Onboarding02() {
  ...
  const { currentUser, currentOnboardingStep, onboardingComplete, updateOnboardingStep } = useAuth();
  ...
  useEffect(() => {
    ...
    if ((currentOnboardingStep || 1) > 2) {
      navigate(`/onboarding/${currentOnboardingStep}`);
    } else if ((currentOnboardingStep || 1) < 2) {
      navigate(`/onboarding/${currentOnboardingStep || 1}`);
    }
  }, [currentUser, currentOnboardingStep, onboardingComplete, navigate, location.pathname]);

  const handleNext = async (e) => {
    e.preventDefault();
    setIsSubmitting(true);
    try {
      const { success } = await updateOnboardingStep({ data: { step2Done: true } });
      if (success) navigate('/onboarding/3');
    } finally {
              ...
              <div className="max-w-md mx-auto">
                ...
                <form>
                  ...
                  <div className="flex items-center justify-between">
                    <Link className="text-sm underline hover:no-underline" to="/onboarding/1">&lt;- Back</Link>
                    <button onClick={handleNext} className="btn bg-indigo-500 hover:bg-indigo-600 text-white ml-auto disabled:opacity-50" disabled={isSubmitting}>
