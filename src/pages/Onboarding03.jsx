...
function Onboarding03() {
  ...
  useEffect(() => {
    ...
    if ((currentOnboardingStep || 1) > 3) {
      navigate(`/onboarding/${currentOnboardingStep}`);
    } else if ((currentOnboardingStep || 1) < 3) {
  ...
  const handleNext = async (e) => {
    ...
    try {
      const { success } = await updateOnboardingStep({ data: { step3Done: true } });
      if (success) navigate('/onboarding/complete');
    } finally {
